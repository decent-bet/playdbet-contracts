...
// Adds private key(s) to the web3 wallet
const addPrivateKeyToWallet = async () => {
    if (privateKey.indexOf(',') === -1)
        web3.eth.accounts.wallet.add(privateKey)
    else if (privateKey.indexOf(',') !== -1)
        privateKey.split(',').forEach(_privateKey => web3.eth.accounts.wallet.add(_privateKey))
    else
        throw new Error('Please provide a valid private key or array of private keys')
    return true
}
